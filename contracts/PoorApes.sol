...
contract PoorApes is ERC721Enumerable, Ownable, ReentrancyGuard {
     ...
     *                                          ^ THIS BIT ^
     */
	constructor(address _priceFeed, int256 _max_supply, string memory _IPFS_JSON_Folder) ERC721("Poor Apes", "POOR") {
        require(bytes(_IPFS_JSON_Folder).length == 46, "IPFS folder incorrect length");
        max_supply = _max_supply;
        priceFeed = AggregatorV3Interface(_priceFeed);
        baseTokenURI = string(abi.encodePacked("https://ipfs.io/ipfs/", _IPFS_JSON_Folder, "/"));
    }
	...
	function mintNFT() public payable returns (uint256) {
        ...
        if (presale) {
            ...
            require(minting_cost(-1) <= int(msg.value), "More ETH required to mint NFT");
        }

        require(_tokenIds.current() < uint(max_supply), "All genesis NFTs minted");
    ...
    function minting_cost(int256 _nft_number) public view returns (int256) {
        ...
        int256 b =   1068500000000000000;  //  1.0685
        int256 c = -10000000000000000000;  // -10
        int256 d =    100000000000000000;  //  0.01
        int256 cost = PRBMathSD59x18.mul(a, b.pow(PRBMathSD59x18.mul(z, (nft_number * 1000000000000000000)) + c)) + d;
	...
	function withdraw() public payable onlyOwner {
        ...
        payable(founder_2).transfer(blance/2);
	}
    ...
    function addToWhiteList(address _addr) public onlyOwner {
        require(_addr != owner(), "The owner can not be added to the whitelist");
