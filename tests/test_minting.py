...
def test_minting_cost_get_more_expensive(contract):
    first_nft_cost_as_int = int(contract.mint_cost())
    for i in range(100):
        print(i)
        ...
        contract.mint(
            {
                "from": minter_acc,
                "value": int(contract.mint_cost()),
            }
        )
    hundredth_nft_cost_as_int = int(contract.mint_cost())
    assert (
...
def test_mint_cost_function_every_hundred_nfts(contract):
    assert int(contract.mint_cost(0)) == int(price_for_first_nft), (
        "The NFT at position has the wrong price (it should be "
        + str(contract.mint_cost(0).to("ether"))
        + ")"
    )
    assert int(contract.mint_cost(99)) == int(price_for_99th_nft), (
        "The NFT at position has the wrong price (it should be "
        + str(contract.mint_cost(99).to("ether"))
        + ")"
    )
    assert int(contract.mint_cost(199)) == int(price_for_199th_nft), (
        "The NFT at position has the wrong price (it should be "
        + str(contract.mint_cost(199).to("ether"))
        + ")"
    )
    assert int(contract.mint_cost(299)) == int(price_for_299th_nft), (
        "The NFT at position has the wrong price (it should be "
        + str(contract.mint_cost(299).to("ether"))
        + ")"
    )
    assert int(contract.mint_cost(399)) == int(price_for_399th_nft), (
        "The NFT at position has the wrong price (it should be "
        + str(contract.mint_cost(399).to("ether"))
        + ")"
    )
    assert int(contract.mint_cost(499)) == int(price_for_499th_nft), (
        "The NFT at position has the wrong price (it should be "
        + str(contract.mint_cost(499).to("ether"))
        + ")"
    )
    assert int(contract.mint_cost(599)) == int(price_for_599th_nft), (
        "The NFT at position has the wrong price (it should be "
        + str(contract.mint_cost(599).to("ether"))
        + ")"
    )
    assert int(contract.mint_cost(699)) == int(price_for_699th_nft), (
        "The NFT at position has the wrong price (it should be "
        + str(contract.mint_cost(699).to("ether"))
        + ")"
    )
    assert int(contract.mint_cost(700)) == int(price_for_700th_nft), (
        "The NFT at position has the wrong price (it should be "
        + str(contract.mint_cost(700).to("ether"))
        + ")"
    )
    assert int(contract.mint_cost(800)) == int(price_for_800th_nft), (
        "The NFT at position has the wrong price (it should be "
        + str(contract.mint_cost(800).to("ether"))
        + ")"
    )
